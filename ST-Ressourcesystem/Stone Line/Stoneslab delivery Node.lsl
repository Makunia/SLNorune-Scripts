...
string TARGET_NAME = "Stoneslab";
integer MAX_COUNT = 300; // Maximum number of objects needed
    ...
    state_entry()
    {
        ...
        for (i = 0; i < num_items; i++)
        {
            ...
            if (item_name != my_script_name)
            {
                ...
                {
                    // Case 1: Exact match (e.g., "Stoneslab")
                    ...
                    else if (llStringLength(item_name) > target_name_len)
                    {
                        ...
                        // If the next character is a space, it's valid (e.g., "Stoneslab 1")
                        if (suffix_char == " ")
                        {
                            is_target_item = TRUE;
                        }
                        // If the next character is a digit (e.g., "Stoneslab1", "Stoneslab2")
                        else
                        {
        ...
        hoverText = "Delivered Stoneslab: " + (string)currentCount + " / " + (string)MAX_COUNT;
    ...
    changed(integer change)
    {
        if ((change & CHANGED_INVENTORY) || (change & CHANGED_ALLOWED_DROP))
        {
            ...
            if (currentCount >= MAX_COUNT)
            {
                ...
                llSetText("Delivered Stoneslab: " + (string)currentCount + " / " + (string)MAX_COUNT +"\n(Goal reached!)", <0.0, 1.0, 0.0>, 1.0);
