// Second Life LSL Script for a Timberwood Dispenser
// This script manages a stock of "Timberwood" items and dispenses them to clicking avatars.
// It now includes a timer to automatically refill the stock over time.

// Global variable to store the current count of Timberwood.
// It starts at 10, representing the initial available stock.
integer g_woodCount = 10;

// The maximum count the Timberwood stock can reach.
integer g_maxWoodCount = 10;
...
default
{
    ...
    state_entry()
    {
        ...
        llSetText("Timberwood: " + (string)g_woodCount + "/" + (string)g_maxWoodCount, <1.0, 0.5, 0.0>, 1.0);
    ...
    touch_start(integer total_number)
    {
        ...
        // Check if there is any Timberwood left to give.
        if (g_woodCount > 0)
        {
            // If yes, decrement the count by 1.
            g_woodCount--;

            // Give one item named 'g_itemName' (Timberwood) from the prim's inventory
            // to the clicking avatar. Make sure the 'Timberwood' item is in the prim's inventory.
    ...
    timer()
    {
        ...
        if (g_woodCount < g_maxWoodCount)
        {
            ...
            // llOwnerSay("Timberwood stock refilled to " + (string)g_woodCount);
        }
        else
        {
