// LSL Maintenance Script for Ironware
// Counts 'Ironware' objects dropped into the prim's inventory and
// automatically reduces the count every 12 hours for 'maintenance'.

// --- Konfigurationsvariablen ---
string TARGET_NAME = "Ironware"; // Der Name des Objekts, das für die Wartung gezählt werden soll
integer MAX_COUNT = 20; // Maximale Anzahl von Objekten, die der Prim für die Wartung aufnehmen kann
integer DECREMENT_AMOUNT = 5; // Menge an Ironware, die für die Wartung verbraucht wird
float MAINTENANCE_INTERVAL = 43200.0; // Intervall für die Wartung in Sekunden (12 Stunden = 43200.0 Sekunden)

// --- Skript-Variablen ---
integer currentCount = 0; // Der aktuelle Zählerstand der Ironware-Objekte
...
updateCountAndText()
{
    ...
    hoverText = "Ironware for Maintenance: " + (string)currentCount + " / " + (string)MAX_COUNT;
...
loadCountFromDescription()
{
    ...
    if (loaded_count >= 0 && loaded_count <= MAX_COUNT) {
        ...
        llSay(0, "Loaded previous Ironware count: " + (string)currentCount); // Gibt eine Nachricht in den Chat aus
    } else {
        currentCount = 0; // Wenn die Beschreibung leer oder ungültig ist, bei 0 beginnen
        llSay(0, "Starting with fresh Ironware count: " + (string)currentCount); // Gibt eine Nachricht in den Chat aus
    }
}
...
recountInventory()
{
    ...
    llSay(0, "Recounted Ironware objects. Current total: " + (string)currentCount);
...
default
{
    ...
    state_entry()
    {
        ...
        llSay(0, "Ironware maintenance script is active. Initial count: " + (string)currentCount); // Bestätigung, dass das Skript aktiv ist
    }
    ...
    changed(integer change)
    {
        if (change & CHANGED_INVENTORY)
        {
            ...
            while (currentCount > MAX_COUNT)
            {
                ...
                if (item_to_remove != "") {
                    llSay(0, "Maximum Ironware supply reached (" + (string)MAX_COUNT + "). Returning excess '" + item_to_remove + "'.");
                    ...
                    llSay(0, "Error: Could not find a Ironware item to remove, despite currentCount (" + (string)currentCount + ") being over MAX_COUNT (" + (string)MAX_COUNT + ").");
    ...
    timer()
    {
        // Wenn Ironware vorhanden ist, die Wartung durchführen
        if (currentCount > 0)
        {
            currentCount -= DECREMENT_AMOUNT; // Reduziert den Zählerstand um den Wartungsbetrag
            llSay(0, "Maintenance consumed " + (string)DECREMENT_AMOUNT + " Ironware. Current count: " + (string)currentCount);
            updateCountAndText(); // Text und Zählerstand aktualisieren und speichern
        } else {
            llSay(0, "No Ironware left for maintenance. Please add more!"); // Benachrichtigt, wenn kein Ironware mehr vorhanden ist
        }
    }
}
